...
var WJNS = (_WJNS = {}, _WJNS[ChainId.JANUS] = /*#__PURE__*/new Token(ChainId.JANUS, '0x0a724DE47B01e2B8DAc55a022651D02326b26e3F', 18, 'WJNS', 'Wrapped JNS'), _WJNS);
...
var Pair = /*#__PURE__*/function () {
  function Pair(tokenAmountA, tokenAmountB) {
    ...
    this.liquidityToken = new Token(tokenAmounts[0].token.chainId, Pair.getAddress(tokenAmounts[0].token, tokenAmounts[1].token), 18, 'OMLT-LP', 'Omelette LP Token');
...
var Route = /*#__PURE__*/function () {
  function Route(pairs, input, output) {
    ...
    !(input instanceof Token && pairs[0].involvesToken(input) || input === JNS && pairs[0].involvesToken(WJNS[pairs[0].chainId])) ? process.env.NODE_ENV !== "production" ? invariant(false, 'INPUT') : invariant(false) : void 0;
    !(typeof output === 'undefined' || output instanceof Token && pairs[pairs.length - 1].involvesToken(output) || output === JNS && pairs[pairs.length - 1].involvesToken(WJNS[pairs[0].chainId])) ? process.env.NODE_ENV !== "production" ? invariant(false, 'OUTPUT') : invariant(false) : void 0;
...
var Trade = /*#__PURE__*/function () {
  function Trade(route, amount, tradeType) {
    ...
    this.inputAmount = tradeType === TradeType.EXACT_INPUT ? amount : route.input === JNS ? CurrencyAmount.jns(amounts[0].raw) : amounts[0];
    this.outputAmount = tradeType === TradeType.EXACT_OUTPUT ? amount : route.output === JNS ? CurrencyAmount.jns(amounts[amounts.length - 1].raw) : amounts[amounts.length - 1];
...
var Router = /*#__PURE__*/function () {
  ...
  Router.swapCallParameters = function swapCallParameters(trade, options) {
    ...
    var jnsOut = trade.outputAmount.currency === JNS;
    // the router does not support both ether in and out
    !!(jnsIn && jnsOut) ? process.env.NODE_ENV !== "production" ? invariant(false, 'JNS_IN_OUT') : invariant(false) : void 0;
...
var TOKEN_DECIMALS_CACHE = (_TOKEN_DECIMALS_CACHE = {}, _TOKEN_DECIMALS_CACHE[ChainId.JANUS] = {
...
var Fetcher = /*#__PURE__*/function () {
  ...
  Fetcher.fetchTokenData = function fetchTokenData(chainId, address, provider, symbol, name) {
    try {
      ...
      return Promise.resolve(_temp ? _temp2(TOKEN_DECIMALS_CACHE[chainId][address]) : Promise.resolve(new Contract(address, ERC20, provider).decimals().then(function (decimals) {
...
export { ChainId, Currency, CurrencyAmount, FACTORY_ADDRESS, Fetcher, Fraction, INIT_CODE_HASH, InsufficientInputAmountError, InsufficientReservesError, JNS, MINIMUM_LIQUIDITY, Pair, Percent, Price, Rounding, Route, Router, Token, TokenAmount, Trade, TradeType, WJNS, currencyEquals, inputOutputComparator, tradeComparator };
